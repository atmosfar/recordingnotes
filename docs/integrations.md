...
Automate session creation and timer control by sending HTTP POST requests to Recording Notes. All integration endpoints require token authentication.

## Authentication

All integration requests must include the `RECNOTES_AUTH_API_TOKEN` (set in `settings.conf` or environment variables). If not explicitly set, the token is auto-generated from your `RECNOTES_AUTH_USERNAME` and `RECNOTES_AUTH_PASSWORD`. The token is printed to the console during server startup.

Pass the token as a query parameter: `?token=YOUR_TOKEN`.
