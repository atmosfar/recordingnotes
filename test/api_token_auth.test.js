...
describe('API Token Authentication', () => {
  ...
  let baseUrl;
  let app;
  const apiToken = 'secret_api_token';

  before(async () => {
    process.env.RECNOTES_AUTH_API_TOKEN = apiToken;
    process.env.RECNOTES_DB_PATH = 'test-api-token-auth.db';
    resetDbInstance();
  ...
  describe('Triggers', () => {
    test('should return 401 for Triggers API without token', async () => {
    ...
    test('should succeed for Triggers API with correct token', async () => {
      const uniqueName = `Trigger ${Date.now()}`;
      const response = await fetch(`${baseUrl}/api/triggers?token=${apiToken}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
